...
// CPT Code Database - Production-level CPT validation system
export const cptDatabase = pgTable("cpt_database", {
  id: serial("id").primaryKey(),
  code: text("code").notNull().unique(),
  description: text("description").notNull(),
  category: text("category").notNull(), // 'evaluation_management', 'surgery', 'radiology', etc.
  ...
  allowedModifiers: text("allowed_modifiers").array().default([]), // ["-25", "-59", "-RT", "-LT"]
  bundledCodes: text("bundled_codes").array().default([]), // Codes that cannot be billed together
  
  // Reimbursement
  baseRate: decimal("base_rate", { precision: 8, scale: 2 }),
  rvu: decimal("rvu", { precision: 6, scale: 2 }), // Relative Value Units
  
  // Data source and updates
  dataSource: text("data_source").default("AMA"), // 'AMA', 'CMS', 'Internal'
  lastVerified: timestamp("last_verified").defaultNow(),
...
// CPT Modifier Database - Production-level modifier validation
export const cptModifiers = pgTable("cpt_modifiers", {
  id: serial("id").primaryKey(),
  modifier: text("modifier").notNull().unique(), // "-25", "-59", "-RT", etc.
  description: text("description").notNull(),
  category: text("category").notNull(), // 'anatomical', 'service', 'provider', 'informational'
  
  // Modifier rules
  canCombineWith: text("can_combine_with").array().default([]), // Compatible modifiers
  cannotCombineWith: text("cannot_combine_with").array().default([]), // Incompatible modifiers
  applicableCptCategories: text("applicable_cpt_categories").array().default([]), // Which CPT categories can use this modifier
  
  // Billing impact
  reimbursementAdjustment: decimal("reimbursement_adjustment", { precision: 5, scale: 4 }).default("1.0000"), // Multiplier (1.5 = 150%, 0.5 = 50%)
  
  // Compliance
  isActive: boolean("is_active").default(true),
  requiresDocumentation: boolean("requires_documentation").default(false),
  
  dataSource: text("data_source").default("AMA"),
  lastVerified: timestamp("last_verified").defaultNow(),
...
// Billing Audit Trail - Athena-style comprehensive audit system
export const billingAuditTrail = pgTable("billing_audit_trail", {
  id: serial("id").primaryKey(),
  
  // Event identification
  eventType: text("event_type").notNull(), // 'cpt_code_added', 'cpt_code_modified', 'modifier_added', 'diagnosis_changed', 'validation_failed'
  entityType: text("entity_type").notNull(), // 'cpt_code', 'diagnosis', 'modifier', 'mapping'
  ...
  afterValue: jsonb("after_value"), // New value
  changeReason: text("change_reason"), // User-provided reason
  reasonCode: text("reason_code"), // Standardized reason codes
  
  // Financial impact
  revenueImpact: decimal("revenue_impact", { precision: 10, scale: 2 }), // Dollar impact of change
  
  // Source tracking
  changedBy: integer("changed_by").references(() => users.id).notNull(),
  changeSource: text("change_source").notNull(), // 'gpt_generated', 'provider_manual', 'system_validation', 'external_import'
  
  // Validation status
  validationStatus: text("validation_status").default("pending"), // 'pending', 'validated', 'flagged', 'rejected'
...
// External Billing System Integration - API endpoints preparation
export const billingSystemIntegrations = pgTable("billing_system_integrations", {
  id: serial("id").primaryKey(),
  
  // Integration identification
  systemName: text("system_name").notNull(), // 'athena', 'epic', 'nextgen', 'eclinicalworks'
  systemType: text("system_type").notNull(), // 'practice_management', 'ehr', 'billing_service', 'clearinghouse'
  
  // Connection details
  apiEndpoint: text("api_endpoint"),
  authType: text("auth_type"), // 'oauth2', 'api_key', 'basic_auth'
  credentials: jsonb("credentials"), // Encrypted credentials
  
  // Sync configuration
  syncDirection: text("sync_direction").notNull(), // 'export_only', 'import_only', 'bidirectional'
  syncFrequency: text("sync_frequency").default("real_time"), // 'real_time', 'hourly', 'daily', 'manual'
  lastSyncAt: timestamp("last_sync_at"),
  
  // Data mapping
  fieldMappings: jsonb("field_mappings").default({}), // Map our fields to external system fields
  
  // Status and monitoring
  isActive: boolean("is_active").default(true),
  connectionStatus: text("connection_status").default("pending"), // 'pending', 'connected', 'error', 'disconnected'
...
// Billing Export Queue - Manage exports to external systems
export const billingExportQueue = pgTable("billing_export_queue", {
  id: serial("id").primaryKey(),
  
  // Export identification
  exportType: text("export_type").notNull(), // 'encounter', 'claim', 'payment', 'adjustment'
  entityId: integer("entity_id").notNull(), // ID of encounter/claim being exported
  patientId: integer("patient_id").references(() => patients.id).notNull(),
  encounterId: integer("encounter_id").references(() => encounters.id),
  
  // Target system
  integrationId: integer("integration_id").references(() => billingSystemIntegrations.id).notNull(),
  
  // Export data
  exportData: jsonb("export_data").notNull(), // Complete data package for export
  exportFormat: text("export_format").notNull(), // 'hl7_fhir', 'x12_edi', 'json', 'xml'
  
  // Status tracking
  status: text("status").default("pending"), // 'pending', 'processing', 'sent', 'acknowledged', 'failed', 'retry'
...
// Billing system Zod schemas
export const insertCptDatabaseSchema = createInsertSchema(cptDatabase).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true,
  lastVerified: true
...
export const insertCptModifiersSchema = createInsertSchema(cptModifiers).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true,
  lastVerified: true
...
export const insertBillingAuditTrailSchema = createInsertSchema(billingAuditTrail).omit({ 
  id: true, 
  createdAt: true
});

export const insertBillingSystemIntegrationsSchema = createInsertSchema(billingSystemIntegrations).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true
});

export const insertBillingExportQueueSchema = createInsertSchema(billingExportQueue).omit({ 
  id: true, 
  createdAt: true, 
  updatedAt: true
});

export type InsertCptDatabase = z.infer<typeof insertCptDatabaseSchema>;
export type SelectCptDatabase = typeof cptDatabase.$inferSelect;
export type InsertCptModifiers = z.infer<typeof insertCptModifiersSchema>;
export type SelectCptModifiers = typeof cptModifiers.$inferSelect;
export type InsertBillingAuditTrail = z.infer<typeof insertBillingAuditTrailSchema>;
export type SelectBillingAuditTrail = typeof billingAuditTrail.$inferSelect;
export type InsertBillingSystemIntegrations = z.infer<typeof insertBillingSystemIntegrationsSchema>;
export type SelectBillingSystemIntegrations = typeof billingSystemIntegrations.$inferSelect;
export type InsertBillingExportQueue = z.infer<typeof insertBillingExportQueueSchema>;
export type SelectBillingExportQueue = typeof billingExportQueue.$inferSelect;
