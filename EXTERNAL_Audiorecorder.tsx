  ...
  const handleStartRecording = async () => {
    ...
    if (!client) {
      ...
      toast({
        ...
        description:
          "WebSocket client is not initialized. Please try again in a moment.",
      });
      return;
    }
    ...
    if (client.eventHandler && client.eventHandler.suggestionsModule) {
      ...
      if (onStartRecording && typeof onStartRecording === "function") {
        try {
          ...
          console.warn(
            "[AudioRecorder] Could not call onStartRecording callback:",
            error,
          );
        }
      }
      ...
      console.warn(
        "[AudioRecorder] Could not access suggestionsModule to unfreeze insights",
      );
    }
    ...
    console.log(
      "[AudioRecorder] Client appears to be:",
    ...
    try {
      ...
      if (!data.available) {
        console.error(
          "[AudioRecorder] OpenAI API key not available on the server",
        );
        toast({
          variant: "destructive",
          title: "API Key Missing",
          description:
            "Unable to access AI services. Please check your API key configuration or contact support.",
        });
        return;
      }
    } catch (error) {
      ...
      if (!client.hasPatientChartBeenSent(patientIdStr)) {
        ...
        try {
          const chartResponse = await fetch(
            `/api/patients/${patientIdStr}/chart`,
          );

          if (!chartResponse.ok) {
            console.error(
              `[AudioRecorder] Error fetching chart: ${chartResponse.status}`,
            );
            throw new Error(
              `Failed to fetch patient chart: ${chartResponse.statusText}`,
            );
          }
      ...
      try {
        ...
        processor.onaudioprocess = async (e) => {
          if (!client || !client.hasValidApiKey()) {
            console.error(
              "[AudioRecorder] Client not available or has no valid API key",
            );
            onStopRecording();
            toast({
              variant: "destructive",
              title: "API Key Error",
              description:
                "API key validation failed. Unable to process audio.",
            });
            return;
          }
          ...
          if (!client.isInitialized()) {
            console.log(
              "[AudioRecorder] Client not initialized, attempting to initialize",
            );
            try {
              await client.init();
            } catch (error) {
              console.error(
                "[AudioRecorder] Failed to initialize client:",
                error,
              );
              onStopRecording();
              toast({
                ...
                title: "Connection Error",
                description:
                  "Failed to initialize WebSocket client. Please try again.",
              });
              return;
            }
          }
    ...
    if (!client.hasPatientChartBeenSent(patientIdStr)) {
      ...
      fetch(`/api/patients/${patientIdStr}/chart`)
        .then((response) => {
          if (!response.ok) {
            console.error(
              `[AudioRecorder] Error fetching chart: ${response.status}`,
            );
            throw new Error(
              `Failed to fetch patient chart: ${response.statusText}`,
            );
          }
        ...
        .catch((error) => {
          console.error(
            "[AudioRecorder] Error processing chart data for text mode:",
            error,
          );
