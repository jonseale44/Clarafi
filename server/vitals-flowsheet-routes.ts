...
const VitalsEntrySchema = z.object({
  ...
  temperature: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) : val).refine(val => !isNaN(val) && val >= 90 && val <= 110, { message: "Temperature must be between 90-110" }).optional(),
  weight: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) : val).refine(val => !isNaN(val) && val >= 1 && val <= 1000, { message: "Weight must be between 1-1000" }).optional(),
  height: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) : val).refine(val => !isNaN(val) && val >= 10 && val <= 100, { message: "Height must be between 10-100" }).optional(),
  bmi: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) : val).refine(val => !isNaN(val) && val >= 10 && val <= 80, { message: "BMI must be between 10-80" }).optional(),
  oxygenSaturation: z.union([z.number(), z.string()]).transform(val => typeof val === 'string' ? parseFloat(val) : val).refine(val => !isNaN(val) && val >= 70 && val <= 100, { message: "Oxygen saturation must be between 70-100" }).optional(),
