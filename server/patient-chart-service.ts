...
export class PatientChartService {
  ...
  static async getPatientChartData(patientId: number): Promise<{
    ...
    try {
      console.log(
        `[Chart Service] Fetching chart data for patient ${patientId}`,
      );
      ...
      const activeProblems = Array.isArray(patient[0]?.activeProblems)
        ? patient[0].activeProblems
        : [];

      // Get medical history
      const medicalHistoryRecords = await db
        .select()
        .from(medicalHistory)
      ...
      // Get allergies
      const allergyRecords = await db
        .select()
        .from(allergies)
        .where(eq(allergies.patientId, patientId));

      // Get medical problems
      const medicalProblemsRecords = await db
        .select()
      ...
      const imagingResultsRecords = await db
        .select()
        .from(imagingResults)
      ...
      console.log(`[Chart Service] Retrieved chart data:`, {
        activeProblems: Array.isArray(chartData.activeProblems)
          ? chartData.activeProblems.length
          : 0,
      ...
      console.error(
        "[Chart Service] Error fetching patient chart data:",
        error,
      );
      return {
        activeProblems: [],
