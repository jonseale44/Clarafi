...
export function MainTranscription({
  ...
  const [isLoadingNote, setIsLoadingNote] = useState<boolean>(
    !!externalEncounterData ? false : (!!encounterId && encounterId !== "new" ? true : false)
  );
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: encounterId && encounterId !== "new" ? 
    ...
    onUpdate: ({ editor }) => {
      ...
      if (!editor.isDestroyed && !isUpdatingProgrammatically.current) {
        ...
        console.log(`[MainTranscription] Editor content changed (${newContent.length} chars)`);
        ...
        if (encounterId && onEncounterChange && externalEncounterData) {
          console.log("[MainTranscription] Updating parent component with new note content");
          const updatedData = {
  ...
  useEffect(() => {
    if (editor) {
      console.log("[MainTranscription] Editor initialized/updated with content length:", 
      ...
      if (soapNote && soapNote.length > 0) {
        ...
        if (editorText.length < 3 && soapNoteText.length > 3) {
          console.log("[MainTranscription] Force-updating editor content on initialization - editor empty but note exists");
          ...
          try {
            ...
            console.log("[MainTranscription] Initial editor content set successfully");
          } catch (err) {
            console.error("[MainTranscription] Error setting initial editor content:", err);
          } finally {
      ...
      if (isLoadingNote && soapNote) {
        console.log("[MainTranscription] Editor initialized with note content, ensuring loading state is reset");
  ...
  useEffect(() => {
    ...
    if (!editor || editor.isDestroyed || isUpdatingProgrammatically.current) {
      console.log("[MainTranscription] Skipping editor update - editor not ready or already updating");
      return;
    }
    ...
    const lengthDifference = Math.abs(currentEditorHTML.length - soapNote.length);
    const stripTags = (html: string): string => html.replace(/<[^>]*>/g, '').trim();
    ...
    const contentChanged = (editorIsEmpty && hasRealContent) || 
    ...
    if (contentChanged) {
      ...
      try {
        ...
        setTimeout(() => {
          isUpdatingProgrammatically.current = false;
          console.log("[MainTranscription] Reset isUpdatingProgrammatically flag");
        }, 100);
      }
    } else {
      console.log("[MainTranscription] No content change needed - editor already has current content");
    }
  }, [soapNote, editor]);
  ...
  useEffect(() => {
    if (encounterId) {
      ...
      if (encounterId !== "new") {
        console.log("[MainTranscription] Encounter ID changed, setting loading state to true");
  ...
  useEffect(() => {
    ...
    if (isLoadingNote) {
      console.log("[MainTranscription] Setting safety timeout for loading state");
      const safetyTimeout = setTimeout(() => {
        console.log("[MainTranscription] Safety timeout triggered, forcing loading state to false");
        setIsLoadingNote(false);
      }, 5000);
  ...
  useEffect(() => {
    ...
    if (externalEncounterData) {
      ...
      console.log(
        "[MainTranscription] Note content check:", {
          hasNote: !!externalEncounterData.note,
          noteLength: externalEncounterData.note ? externalEncounterData.note.length : 0,
          notePreview: externalEncounterData.note ? externalEncounterData.note.substring(0, 50) + "..." : "(empty)"
        }
      );
      ...
      if (externalEncounterData.note) {
        console.log("[MainTranscription] Setting note content from encounters.note (canonical location)");
        ...
        console.log("[MainTranscription] No note content found in encounters.note, using empty string");
        setSoapNote("");
      }
    ...
    if (encounterId) {
      ...
      const fetchEncounterData = async () => {
        try {
          ...
          console.log(`[MainTranscription] Fetching encounter data for patient ${patientId}, encounter ${encounterId}`);

          // Additional debug info about API request
          console.log(`[MainTranscription] API URL: /api/patients/${patientId}/office-visits/${encounterId}`);
          ...
          console.log(`[MainTranscription] API response status: ${response.status} ${response.statusText}`);

          if (!response.ok) {
            console.error(`[MainTranscription] Error fetching encounter: ${response.status} ${response.statusText}`);
            throw new Error(`Failed to fetch encounter data: ${response.status} ${response.statusText}`);
          }

          // Get response as text first to inspect it
          const responseText = await response.text();
          console.log(`[MainTranscription] Raw API response length: ${responseText.length}`);
          console.log(`[MainTranscription] Raw API response (first 500 chars): ${responseText.substring(0, 500)}...`);
          ...
          try {
            ...
            console.log("[MainTranscription] ENCOUNTER STRUCTURE DEBUG:", JSON.stringify({
            ...
            console.error(`[MainTranscription] JSON parse error: ${parseError}`);
          ...
          console.log("[MainTranscription] Note content check:", {
            ...
            notePreview: encounter.note ? encounter.note.substring(0, 50) + "..." : "(empty)",
            noteType: typeof encounter.note,
            isNullOrUndefined: encounter.note === null || encounter.note === undefined,
          ...
          if (setOriginalEncounterData) {
            console.log("[MainTranscription] Saving original encounter data to parent component");
  ...
  const saveMutation = useMutation({
    mutationFn: async (soapNote: string) => {
      ...
      console.log(`[MainTranscription] CRITICAL FIX - Using direct canonical URL: ${url}`);
      ...
      if (externalEncounterData?._original_state === "signed" || externalEncounterData?._original_encounter_status === "signed") {
        console.log(
    ...
    switch (event.type) {
      ...
      case "gpt.analysis.delta":
        ...
        if (areInsightsFrozen) {
          console.log("[MainTranscription] GPT analysis update skipped - insights are frozen");
          break;
        }
        setGptAnalysis((prev) => {
        ...
        if (event.note) {
          ...
          if (event.type === "soap.note.completed") {
            ...
            if (webSocketClient?.eventHandler?.cptCodesModule && event.note) {
              console.log("[MainTranscription] Scanning completed SOAP note for CPT codes");
              webSocketClient.eventHandler.cptCodesModule.parseTextForCPTCodes(event.note);
            } else {
              console.log("[MainTranscription] Cannot scan for CPT codes - module not available");
            }
...
Include ICD-10 codes for all conditions immediately after each condition in the Assessment/Plan section.
  ...
  const handleStopRecording = async () => {
    ...
    if (webSocketClient?.eventHandler?.suggestionsModule) {
      ...
      console.log("[MainTranscription] Called freezeInsights() on suggestionsModule");
    } else {
      console.warn("[MainTranscription] Could not freeze insights - suggestionsModule not available");
    }
      ...
      try {
        ...
        if (soapNote && patientId && encounterId) {
          ...
          if (webSocketClient?.eventHandler?.diagnosisCPTMapper) {
            const diagnosisCPTMapper = webSocketClient.eventHandler.diagnosisCPTMapper;
            ...
            console.log("[MainTranscription] Successfully processed SOAP note for CPT codes");
          } else {
            console.warn("[MainTranscription] DiagnosisCPTMapper module not available");
          }
        } else {
          console.warn("[MainTranscription] Cannot process CPT codes - missing SOAP note or patient/encounter ID");
        }
      } catch (error) {
        ...
        <AudioRecorder
          ...
          onStartRecording={() => {
            ...
            if (webSocketClient?.eventHandler?.suggestionsModule) {
              ...
              console.log("[MainTranscription] Called unfreezeInsights() on suggestionsModule");
            }
          }}
          patientId={patientId}
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (!currentContent || currentContent.length < 10) {
                            toast({
                              variant: "destructive",
                              description: "Please add some content to your SOAP note first"
                            });
                            return;
                          }

                          toast({
                            description: "Generating smart suggestions with patient memory...",
                            duration: 3000
                          });

                          // Call the Assistant API for smart SOAP note suggestions
                          const response = await fetch(`/api/assistant/soap-note`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                          ...
                          if (result.soapNote && editor) {
                            ...
                            toast({
                              description: "SOAP note enhanced with patient memory insights!",
                              variant: "default"
                            });
                          }
                        } catch (error) {
                    ...
                    {editor && soapNote && soapNote.length > 0 ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-full w-full p-4">
                        <div className="text-center space-y-3">
                          ...
                          <p className="text-muted-foreground">Loading existing note content...</p>
                          {encounterId && encounterId !== "new" && (
                      ...
                      <div className="p-4">
                        <p className="text-muted-foreground">Editor is loading...</p>
                    ...
                    {process.env.NODE_ENV !== 'production' && (
                      <div className="mt-2 p-2 text-xs text-gray-500 border-t">
                        <details>
                          ...
                          <p>Editor HTML length: {editor?.getHTML()?.length || 0}</p>
                          <p>Loading state: {isLoadingNote ? 'Loading' : 'Complete'}</p>
                          <p>Encounter ID: {encounterId || 'None'}</p>
                          <p>API endpoint: {`/api/patients/${patientId}/office-visits/${encounterId}`}</p>
                        </details>
                      </div>
                    )}
                  </div>
