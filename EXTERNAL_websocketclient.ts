...
export class WebSocketClient {
  ...
  constructor(
    ...
    if (typeof apiKeyOrConfig === "object") {
      ...
      console.log(
        "[WebSocketClient] Config contains patientId:",
        !!apiKeyOrConfig.patientId,
      );
      console.log(
        "[WebSocketClient] Config contains encounterId:",
        !!apiKeyOrConfig.encounterId,
      );
      console.log(
        "[WebSocketClient] Config contains API key:",
        !!apiKeyOrConfig.apiKey,
      );
      console.log(
        "[WebSocketClient] Has transcript update handler:",
        !!apiKeyOrConfig.onTranscriptUpdate,
      );
      console.log(
        "[WebSocketClient] Has GPT update handler:",
        !!apiKeyOrConfig.onGPTUpdate,
      );
    } else {
    ...
    if (typeof apiKeyOrConfig === "string") {
      ...
      if (apiKeyOrConfig.apiKey) {
        ...
        console.log(
          "[WebSocketClient] Falling back to environment variable for API key",
        );
      ...
      if (apiKeyOrConfig.prompt) {
        ...
        this.prompt =
          "You are a helpful medical assistant trained to assist with medical documentation. You will analyze patient encounters and provide structured medical notes.";
      }
      ...
      this.onMessage = (event: any) => {
        ...
        if (
          event.type === "response.transcript" ||
          ...
          const transcriptContent =
            event.transcript || event.delta?.text || event.delta || "";

          // Log detailed debug information
          console.log(
            `[WebSocketClient] Received transcript event: ${event.type}`,
            transcriptContent.substring(0, 30) + "...",
          );

          // Send to callback if available
          if (this.onTranscriptUpdate) {
            console.log(
              "[WebSocketClient] Calling onTranscriptUpdate callback",
            );
            this.onTranscriptUpdate(transcriptContent);
          } else {
            console.warn(
              "[WebSocketClient] No onTranscriptUpdate callback available",
            );
          }
        }
        // Handle all GPT analysis event types but check if it's not a SOAP note
        else if (
          (event.type === "response.text" ||
            event.type === "gpt.analysis.delta") &&
          !(
            event.metadata?.type === "soap_note" ||
            event.response?.metadata?.type === "soap_note"
          )
        ) {
          ...
          // Log detailed debug information
          console.log(
            `[WebSocketClient] Received GPT event: ${event.type}`,
        ...
        // Handle SOAP note events from both direct soap.note events and response.text with soap_note metadata
          ...
          console.log(
            `[WebSocketClient] Received SOAP note event: ${event.type}`,
          );
          ...
          if (event.type === "response.text") {
            if (event.response?.status === "done") {
              ...
              console.log(
                "[WebSocketClient] Transformed response.text to soap.note.completed",
              );
            } else {
              ...
              console.log(
                "[WebSocketClient] Transformed response.text to soap.note.delta",
              );
            }
          }
          ...
          if (apiKeyOrConfig.onMessage) {
            console.log(
              "[WebSocketClient] Forwarding SOAP note event to onMessage callback",
            );
            apiKeyOrConfig.onMessage(transformedEvent);
          } else {
            console.warn(
              "[WebSocketClient] No onMessage callback available for SOAP note event",
            );
          }
        }
      };
  ...
  public hasValidApiKey(): boolean {
    ...
    console.log(
      "[WebSocketClient] API key validity check:",
  ...
  public isInitialized(): boolean {
    ...
    console.log(
      "[WebSocketClient] Initialization status:",
  ...
  async init() {
    console.log(
      "[WebSocketClient] Initializing with API key:",
      this.apiKey ? "Present (masked)" : "Missing",
    );

    // Check API key from environment and current instance
    const envApiKey = import.meta.env.VITE_OPENAI_API_KEY;
    console.log(
      "[WebSocketClient] Environment API key check:",
      envApiKey ? "Present in env" : "Missing from env",
    );
    console.log(
      "[WebSocketClient] Instance API key length:",
    ...
    if (!this.apiKey) {
      console.log(
        "[WebSocketClient] Attempting to fetch API key status from server",
      );
      try {
        ...
        if (data.available) {
          console.log(
            "[WebSocketClient] Server reports API key is available, but client doesn't have access",
          );
          console.log(
            "[WebSocketClient] This suggests an environment configuration issue",
          );
        }
      } catch (error) {
        console.error(
          "[WebSocketClient] Failed to check API key status from server:",
          error,
        );
      }

      console.error(
        "[WebSocketClient] No API key available. Cannot initialize session.",
      );
  ...
  markPatientChartAsSent(patientId: string | number): void {
    ...
    console.log(
      `[WebSocketClient] Patient chart ${patientIdStr} marked as sent`,
    );
  }
  ...
  public requestSOAPNote(instructions: string): void {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.error(
        "[WebSocket] Cannot send SOAP note request: WebSocket not connected.",
      );
      return;
    }
    ...
    if (!this.eventHandler) {
      console.error(
        "[WebSocket] Cannot generate SOAP note: Missing EventHandler",
      );
      return;
    }
